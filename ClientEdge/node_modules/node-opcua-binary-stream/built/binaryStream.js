 ...
 */
var better_assert_1 = require("better-assert");
...
function BinaryStream(data) {
    if (data === undefined) {
        ...
        better_assert_1.assert(data instanceof Buffer);
        this._buffer = data;
    }
    this.length = 0;
}
...
BinaryStream.prototype.writeInt8 = function (value) {
    better_assert_1.assert(value >= -128 && value < 128);
...
BinaryStream.prototype.writeUInt8 = function (value) {
    better_assert_1.assert(value >= 0 && value < 256 && " writeUInt8 : out of bound ");
...
var my_memcpy = function my_memcpy(target, targetStart, source, sourceStart, sourceEnd) {
    better_assert_1.assert(target instanceof Buffer || target instanceof Uint8Array);
    better_assert_1.assert(source instanceof Buffer || source instanceof Uint8Array);
    var l = targetStart;
...
function display_memcpy_missing_message() {
    console.warn(chalk_1.chalk.yellow("\n Warning : the memcpy package is not installed on your system "));
    console.warn(chalk_1.chalk.yellow("\n           memcpy could allow you to get better encoding/decoding performance on typed array "));
    console.warn(chalk_1.chalk.yellow("             you can install it memcpy using this command ( extra c++ compilation tools may be required on your system)"));
    console.warn(chalk_1.chalk.yellow("              $ npm install  memcpy"));
    console.warn("");
}
// note :
...
try {
    ...
    my_memcpy = memcpy;
    console.log(chalk_1.chalk.yellow("Warning : using memcpy : OK"));
}
catch (err) {
    if (displayWarnings) {
...
BinaryStream.prototype.writeArrayBuffer = function (arrayBuf, offset, length) {
    offset = offset || 0;
    better_assert_1.assert(arrayBuf instanceof ArrayBuffer);
...
BinaryStream.prototype.readArrayBuffer = function (length) {
    better_assert_1.assert(this.length + length <= this._buffer.length, "not enough bytes in buffer");
...
BinaryStream.prototype.writeByteStream = function (buf) {
    ...
    better_assert_1.assert(buf instanceof Buffer);
